        ...
        <Card className={"p-2 " + (justUpdated ? 'ring-1 ring-primary/40 rounded-md transition-shadow shadow-[0_0_0_3px_rgba(59,130,246,0.15)]' : '')}>
            <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                    ...
                    {justUpdated && <span className="text-[10px] px-2 py-0.5 rounded bg-primary/10 text-primary">{t('updated')}</span>}
                </div>
                <div className="flex items-center gap-2">
                    <Button size="sm" variant={view === 'table' ? 'default' : 'outline'} onClick={() => setView('table')}>{t('table')}</Button>
                    <Button size="sm" variant={view === 'raw' ? 'default' : 'outline'} onClick={() => setView('raw')}>{t('raw')}</Button>
                    <Button
                        variant="outline"
                        size="sm"
                        onClick={async () => {
                    ...
                    <Button variant="outline" size="sm" onClick={onDownload} title={t('downloadCsv')}>
                ...
                <div className="overflow-auto border rounded max-h-[400px]">
                    <table className="w-full border-collapse text-sm">
                        ...
                        <tbody>
                            {dataRows.length === 0 ? (
                                <tr>
                                    <td className="px-2 py-2 text-muted-foreground" colSpan={Math.max(1, headers.length)}>{t('noData')}</td>
                                </tr>
                            ) : (

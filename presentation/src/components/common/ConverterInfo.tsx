    ...
    return (
        <div className="mb-2">
            <Button variant="ghost" size="sm" onClick={() => setOpen(o => !o)} className="text-muted-foreground">
                ...
                <span>{open ? t('hideDetails') : t('showConverterInfo')}</span>
                {open ? <ChevronDown className="h-4 w-4 ml-2" /> : <ChevronRight className="h-4 w-4 ml-2" />}
            </Button>
            {open && (
                <Card className="mt-2">
                    <CardContent className="space-y-3 pt-4">
                        <p className="text-sm text-muted-foreground">{t(String(usage.summary))}</p>
                        <div className="space-y-3">
                            {usage.useCases.map((uc, i) => (
                                <div key={i} className="rounded-md border p-3">
                                    <div className="font-medium text-sm">{t(String(uc.title))}</div>
                                    <p className="text-sm text-muted-foreground mt-1">{t(String(uc.description))}</p>
                                        ...
                                        <div className="grid gap-3 md:grid-cols-2 mt-3 text-xs font-mono">
                                            {uc.exampleInput && (
                                                <div className="rounded bg-muted/50 p-2 overflow-auto">
                                                    <div className="text-[10px] uppercase text-muted-foreground mb-1">{t('exampleInputLabel')}</div>
                                                    <pre className="whitespace-pre-wrap">{uc.exampleInput}</pre>
                                                </div>
                                            )}
                                            {uc.exampleOutput && (
                                                <div className="rounded bg-muted/50 p-2 overflow-auto">
                                                    <div className="text-[10px] uppercase text-muted-foreground mb-1">{t('exampleOutputLabel')}</div>

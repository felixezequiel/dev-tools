...
export class JsonToSqlUpdateBuilder implements JsonOutputBuilder<string> {
    ...
    private buildSingleUpdate(data: Record<string, any>): string {
        ...
        if (!whereClause && this.options.primaryKey && data[this.options.primaryKey] !== undefined) {
            const pkValue = this.formatValue(data[this.options.primaryKey]);
            whereClause = `${this.escapeIdentifier(this.options.primaryKey)} = ${pkValue}`;
        }
    ...
    private buildBulkUpdate(data: Record<string, any>[]): string {
        ...
        data.forEach(item => {
            const pkValue = item[this.options.primaryKey!];
            if (pkValue === undefined) {
                throw new Error(`Primary key ${this.options.primaryKey} is required for all items in bulk update`);
            }
            ...
            if (updates.length > 0) {
                const whereClause = `${this.escapeIdentifier(this.options.primaryKey)} = ${this.formatValue(pkValue)}`;
